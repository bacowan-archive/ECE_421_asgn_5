...
class DatabaseProxy
  ...
  def saveGame(gameName,game)
    @db.query('INSERT INTO ' + SAVED_GAMES_TABLE_NAME + " (" + GAME_NAME_COLUMN + ", " + GAME_COLUMN + ") VALUES ('" + gameName + "', '" + @db.escape_string(game) + "') " +
      "ON DUPLICATE KEY UPDATE " + GAME_COLUMN + "='" + @db.escape_string(game) + "'")
  end
